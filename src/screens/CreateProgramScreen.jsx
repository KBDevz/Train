    ...
    return (
      <div className="screen-container px-4 pt-4">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h1 className="text-lg font-bold text-text">Create Program</h1>
        </div>

        <div className="space-y-3">
          <button
            onClick={() => setMode('manual')}
            className="w-full bg-card rounded-xl p-5 text-left border-2 border-border active:bg-card-hover"
          >
            <p className="font-semibold text-text mb-1">Manual Program</p>
            <p className="text-sm text-muted">Build your own program from scratch</p>
          </button>

          <button
            onClick={() => { setMode('ai'); handleAiGenerate() }}
            className="w-full bg-gradient-to-r from-primary to-purple-500 rounded-xl p-5 text-left text-bg"
          >
  ...
  return (
    <div className="screen-container px-4 pt-4">
      <div className="flex items-center gap-2 mb-4">
        <button onClick={() => setMode(null)} className="w-9 h-9 flex items-center justify-center rounded-lg bg-card border border-border text-text">
          <ChevronLeft size={20} />
        </button>
        <h1 className="text-lg font-bold text-text">Manual Program</h1>
      </div>

      <div className="space-y-4">
        <div>
          <label className="block text-xs uppercase tracking-wider text-text-secondary mb-1">Program Name</label>
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            className="w-full h-12 px-4 rounded-xl border border-border bg-card text-text text-base focus:outline-none focus:ring-2 focus:ring-primary/30"
            placeholder="e.g. Push Pull Legs"
          />
        </div>

        <div>
          <label className="block text-xs uppercase tracking-wider text-text-secondary mb-1">Description</label>
          <textarea
            value={description}
            onChange={(e) => setDescription(e.target.value)}
            className="w-full h-20 px-4 py-3 rounded-xl border border-border bg-card text-text text-base resize-none focus:outline-none focus:ring-2 focus:ring-primary/30"
            placeholder="Optional description"
          />
        </div>

        <div>
          <label className="block text-xs uppercase tracking-wider text-text-secondary mb-1">Duration (weeks)</label>
          <input
            type="number"
            inputMode="numeric"
            value={weeks}
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-xs uppercase tracking-wider text-text-secondary">Workout Days</label>
          ...
          {workoutDays.map((day, dayIdx) => (
            <div key={dayIdx} className="bg-card rounded-xl p-3 mb-2 border border-border">
              <input
                value={day.name}
                onChange={(e) => {
                  const updated = [...workoutDays]
                ...
                className="w-full h-10 px-3 rounded-lg border border-border bg-subtle text-text text-sm font-medium mb-2 focus:outline-none focus:ring-2 focus:ring-primary/30"
              />

              {day.exercises.map((ex, exIdx) => (
                <div key={exIdx} className="flex items-center gap-2 py-1.5 border-t border-border">
                  <span className="text-sm text-text flex-1">{ex.exercise_name}</span>
                  <input
                    type="number"
                    inputMode="numeric"
                    ...
                    className="w-12 h-8 text-center rounded border border-border bg-subtle text-text text-sm focus:outline-none"
                    placeholder="Sets"
                  />
      ...
      {showExSearch !== null && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end">
          <div className="w-full max-w-[480px] mx-auto bg-card rounded-t-2xl max-h-[70vh] flex flex-col border-t border-border">
            <div className="p-4 border-b border-border flex items-center gap-2">
              <Search size={18} className="text-muted" />
              <input
                value={exSearch}
                onChange={(e) => setExSearch(e.target.value)}
                className="flex-1 text-base bg-transparent text-text focus:outline-none"
            ...
            <div className="overflow-y-auto flex-1 p-2">
              {filteredExercises.map((ex) => (
                <button
                  ...
                  className="w-full text-left px-3 py-3 rounded-lg active:bg-card-hover transition-colors"
                >
