...
export default function HistoryScreen() {
  ...
  return (
    <div className="screen-container px-4 pt-6">
      <h1 className="text-xl font-bold text-text mb-4">History</h1>
              ...
              <div key={s.id} className="bg-card border border-border rounded-xl overflow-hidden">
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <p className="font-semibold text-text text-sm">{s.workout_days?.name || 'Workout'}</p>
                  ...
                  <div className="flex gap-4 text-xs">
                    <span className="text-text-secondary">⏱ {formatDuration(s.duration_seconds)}</span>
                    <span className="text-text-secondary">📊 {convertWeight(totalVol).toLocaleString()} {unitLabel}</span>
                    <span className="text-text-secondary">✓ {setsCompleted} sets</span>
                  </div>
                      ...
                      <div key={name} className="mt-3">
                        ...
                        {sets.sort((a, b) => a.set_number - b.set_number).map((set) => (
                          <div key={set.id} className="flex items-center gap-2 text-xs py-0.5">
                            ...
                            <span className={`${set.completed ? 'text-text-secondary' : 'text-muted line-through'}`}>
